...
import { Telegraf, Markup, Context as TelegrafContext, Scenes, session as telegrafSession } from 'telegraf';
...
bot.command('lang', async (ctx) => {
  await ctx.reply('Выберите язык / Choose your language / 言語を選択してください / Elige tu idioma / 请选择语言:',
    {
      reply_markup: {
...
bot.action('order_dev', async (ctx) => {
  const MESSAGES = getMessages(ctx);
  await ctx.reply(MESSAGES.orderDev(SUPPORT_CHAT_USERNAME), { parse_mode: 'HTML' });
});
...
bot.on(['text', 'photo', 'video', 'document'], async (ctx) => {
  if (ctx.session && ctx.session.step === 'ask_message') {
    ...
    if (msg.photo && Array.isArray(msg.photo)) {
      ...
      await ctx.telegram.sendPhoto(NOTIFY_CHAT, photo.file_id, { caption: fullCaption, parse_mode: 'HTML' });
    } else if (msg.video) {
      await ctx.telegram.sendVideo(NOTIFY_CHAT, msg.video.file_id, { caption: fullCaption, parse_mode: 'HTML' });
    } else if (msg.document) {
      await ctx.telegram.sendDocument(NOTIFY_CHAT, msg.document.file_id, { caption: fullCaption, parse_mode: 'HTML' });
    } else {
...
bot.action(/set_ticket_status\|([^|]+)\|(\d+)/, async (ctx) => {
  ...
  if (notifyText && userId) {
    try {
      await ctx.telegram.sendMessage(userId, notifyText, { parse_mode: 'HTML' });
    } catch (e) {
